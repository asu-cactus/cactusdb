...
class MultiLayerUDF2TorchNNRewriteActionTest : public HiveConnectorTestBase {
 public:
  ...
  void testMultiLayerUDF2TorchNNPlan(bool rewrite) {
    ...
    std::vector<int> col1 = {1, 2, 3, 4, 5, 6};
    std::vector<int> col2 = {11, 22, 33, 44, 55, 66};

    auto rowVector = maker.rowVector(
        {"c1", "c2"},
        {maker.flatVector(col1, INTEGER()), maker.flatVector(col2, INTEGER())});
        ...
        exec::test::PlanBuilder(planNodeIdGenerator, pool_.get())
            ...
            .nestedLoopJoin(
                exec::test::PlanBuilder(planNodeIdGenerator, pool_.get())
                    ...
                    .planNode(),
                {"v", "c1", "c2"})
            .project({fmt::format(compute, "v") + " AS result", "c1", "c2"})
            .project({"result", "c1", "c2"})
            .planBuild();
    ...
    if (rewrite) {
      ...
      std::pair<std::string, std::string> testAction = std::make_pair(
          "softmax8(mat_add7(mat_mul6(relu5(mat_add4(mat_mul3(relu2(mat_add1(mat_mul0(ROW[\"v\"])))))))))",
