...
def run_cpp_program(path, params):
    # return 0
    execution_command = [
        "/home/ubuntu/velox/build/velox/optimizer/benchmarkFFNN_test {}".format(params)
    ]
    result = float(
...
if __name__ == "__main__":
    ...
    list_eigen_threads = [1,2,4,8]
    list_torch_threads = [1,2,4,8]
    # list_blocks = [4,8,16,32,64,128,256,512,1024]
    list_blocks = [8]
    list_option =[1,2,3]
    num_repeat = 1
    # list_batch_size = ["100", "1000", "10000", "100000"]
    list_batch_size = ["100", "1000", "10000"] # kPreferredOutputBatchBytes 2500000 (597540*4) bytes per item, until 10000 *2500000, then kMaxOutputBatchRows determine the output, we all set it as 250000000000 to make kMaxOutputBatchRows control it.
    ...
    for config in tqdm(run_configs):
        ...
        result_df.to_csv("FFNN_threads_embedding_op2_test.csv", index=False)
