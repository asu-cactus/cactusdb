...
if __name__ == "__main__":
    ...
    run_configs = list(
        itertools.product(
            list_model, list_batch_size, list_feature_size, list_l1_size
        )
    )
    result_df = None
    for config in tqdm(run_configs):
        model, batch_size, feature_size, l1_size = config
        params_base = "-mode=TorchNN -model={} -batch_size={} -feature_size={} -num_repeat={} -l1size={}".format(
            model, batch_size, feature_size, num_repeat, l1_size
        )
        try:
        ...
        result_df.to_csv("result_benchmark_torchnn_kernel.csv", index=False)
