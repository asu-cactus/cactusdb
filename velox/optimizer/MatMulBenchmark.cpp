...
*/
   ...
   */
  std::string registerFunctions(int units1, int input_size1, float* weightsFile_1, CataLog &catalog, bool isVerticalPartition) {
  ...
  std::string registerFunctions(int units1, int input_size1, float* weightsFile_1, CataLog &catalog) {
    // Register matrix multiplication function for the first layer
    bool isVerticalPartition = false;
    optimization::registerVectorFunction(
        "mat_mul0",
        MatrixMultiply_Torch::signatures(),
        std::make_unique<MatrixMultiply_Torch>(weightsFile_1, input_size1, units1),
        {},
        true,
        catalog,
        isVerticalPartition
  ...
  */
  void testMul2JoinAggPlan(bool rewrite, int flag, int samplesSize, int featuresSize, int outputSize) {
    ...
    else if (flag == 2)
    {
      ...
      // Print possible actions
        ...
        //velox-torch for maxtrix multiplication plan
        bool isVerticalPartition = false;
        std::string compute = registerFunctions(
        first_layer_output_size, 
        input_features_size, 
...
int main(int argc, char** argv) {
  ...
  int number0 = std::atoi(argv[1]);//actions
  int number1 = std::atoi(argv[2]);//sample size
  int number2 = std::atoi(argv[3]);//feature size
  int number3 = std::atoi(argv[4]);//output size
  ...
  if (rewrite) {
    ...
    demo.testMul2JoinAggPlan(true, number0, number1, number2, number3);
    ...
    demo.testMul2JoinAggPlan(false, number0, number1, number2, number3);
  }
