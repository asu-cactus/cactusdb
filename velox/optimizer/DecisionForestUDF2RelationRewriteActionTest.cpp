...
class DecisionForestUDF2RelationRewriteActionTest
 ...
 public:
  ...
  parseCSVFile(std::string filePath, int numRows, int numCols) {
  ...
  loadData(std::string& path, int numRows, int numCols, int numSplits = 8) {
  ...
  void runDecisionForestPlan(
      ...
      if (!noMoreSplits) {
        for (const auto& entry : idFileAddrMap) {
          ...
          for (const auto& addr : fileAddr) {
            if (!fs::exists(addr)) {
              LOG(ERROR) << "[ERROR] File not exists: " << addr
                         << " planNode key: " << key
                         << " format: " << fileFormat << std::endl;
              return;
            }
          }
    ...
    ss << "numThreads:" << numThreads << ", numBatch:" << totalNumBatch
       << ", numData:" << totalNumData << std::endl;
...
int main(int argc, char** argv) {
  ...
  demo.testRewriteDecisionForestUDFPlan(
      modelPath, dataPath, rewrite, numSplits, numTreeSplit, batchRows);
}
