...
def evaluate_model2vec(model, ds, bs, norm, device, prints=False):
    model.eval()
...
def train_model2vec(
    ...
    for epoch in range(epochs):
        ...
        if epoch > 10:
            test_scores, corrs = evaluate_model2vec(
...
def train_model2vec_with_contrastive(
    model,
    train_ds,
    val_ds,
    crit,
    ...
    for epoch in range(epochs):
        ...
        for idxs in chunks(train_idxs, bs):
            ...
            similar_models_batch, _ = collator(
                list(zip(*[train_ds[train_ds.similar_model_idx[j]] for j in idxs]))
            )
            disimilar_models_batch, _ = collator(
                list(zip(*[train_ds[train_ds.dissimilar_model_idx[j]] for j in idxs]))
            )
