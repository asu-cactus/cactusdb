...
class FeatureEmbed(nn.Module):
    def __init__(
        ...
        # self.mlOpEmbed = nn.Embedding(num_ml_ops, embed_size)
        # TODO: assume 500 is the max number of operators at one expression
        # self.numOpEmbed = nn.Embedding(500, embed_size)
        # version 1
        # self.linearOpComplexity = nn.Linear(2, embed_size)
        # version 2
        self.linearOpComplexity = nn.Linear(50, embed_size)

        self.linearModelEmbed = nn.Linear(model_embed_size, embed_size)

        self.linearFilter2 = nn.Linear(
            embed_size + embed_size // 8 + 1, embed_size + embed_size // 8 + 1
        )
        ...
        # if use_sample:
        #     self.linearSample = nn.Linear(1000 + 2, embed_size)
        # else:
        self.linearSample = nn.Linear(2, embed_size)
    ...
    def forward(self, feature):
        ...
        # typeId, joinId, filtersId, filtersMask, numOps, leastComplOp, mostComplOp, opCompl, hists, table_sample = torch.split(feature,(1,1,9,3,1,1,1,2,self.bin_number*3,1001+2), dim = -1)
        # version 2
        if self.use_sample:
            (
                typeId,
                ...
                feature,
                (1, 1, 9, 3, self.model_embed_size, self.bin_number * 3, 1001 + 2),
                # (1, 1, 9, 3, 1, 1, 1, 50, self.bin_number * 3, 1001 + 2),
                dim=-1,
            )
        else:
            (
                ...
                feature,
                (1, 1, 9, 3, self.model_embed_size, self.bin_number * 3, 1 + 2),
                dim=-1,
            )

        typeEmb = self.getType(typeId)  # embed
        joinEmb = self.getJoin(joinId)  # embed
        filterEmbed = self.getFilter(filtersId, filtersMask)
        modelLinearEmbed = self.getModelEmbed(modelEmbed)  # embed
        # numOpsEmb = self.getNumOps(numOps)
        # leastComplOpEmb = self.getMLOp(leastComplOp)
        # mostComplOpEmb = self.getMLOp(mostComplOp)
        # opComplEmb = self.getLinearOpComplexity(opCompl)
    ...
    def getModelEmbed(self, modelEmbed):
        emb = self.linearModelEmbed(modelEmbed.float())
        return emb.squeeze(1)

    # def getNumOps(self, numOps):
    #     emb = self.numOpEmbed(numOps.long())
    #     return emb.squeeze(1)

    # def getMLOp(self, mlOp):
    #     emb = self.mlOpEmbed(mlOp.long())
    #     return emb.squeeze(1)

    # def getLinearOpComplexity(self, opCompl):
    #     emb = self.linearOpComplexity(opCompl)
    #     return emb.squeeze(1)
    ...
    def getTable(self, table_sample):
        if self.use_sample:
            table, sample = torch.split(table_sample, (1, 1000 + 2), dim=-1)
            emb = self.tableEmbed(table.long()).squeeze(1)
            emb += self.linearSample(sample)
        else:
            table, sample = torch.split(table_sample, (1, 2), dim=-1)
            emb = self.tableEmbed(table.long()).squeeze(1)
            emb += self.linearSample(sample)
        return emb
...
class QueryFormer(nn.Module):
    def __init__(
        ...
        self.embbed_layer = FeatureEmbed(
            embed_size=emb_size,
            model_embed_size=model_embed_size,
    ...
    def forward(self, batched_data):
        ...
        # version 1
        # x_view = x.view(-1, 1172)
        # version 2
        if self.use_sample:
            x_view = x.view(-1, 1220)
        else:
            x_view = x.view(-1, 359)
