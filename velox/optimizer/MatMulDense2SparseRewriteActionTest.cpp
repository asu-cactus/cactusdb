...
// Custom headers
#include "CataLog.h"
#include "MatMulDense2SparseRewriteAction.h"
#include "PlanState.h"
#include "Register.h"
...
class MatMulDense2SparseRewriteActionTest : public HiveConnectorTestBase {
 public:
  MatMulDense2SparseRewriteActionTest() {
   ...
   */
  void testDense2SparsePlan(bool rewrite) {
    ...
    auto data = data_generate_multi(
        input_features_size,
        num_samples,
        first_layer_output_size,
        second_layer_output_size,
        third_layer_output_size);
    ...
    if (rewrite) {
      ...
      std::pair<std::string, std::string> testAction =
          std::make_pair("mat_mul0(ROW[\"v\"])", "MatMulDense2SparseRewriteAction");
...
int main(int argc, char** argv) {
  ...
  MatMulDense2SparseRewriteActionTest demo;
  ...
  if (rewrite) {
    ...
    demo.testDense2SparsePlan(true);
    ...
    demo.testDense2SparsePlan(false);
  }

  std::cout
      << "--" << std::endl
      << "[Usage] " << std::endl
      << "./_build/release/velox/optimizer/dense2sparse_test Y  //run FFNN model with rewriting rule 2"
      << std::endl
      << "./_build/release/velox/optimizer/dense2sparse_test N  //run FFNN model with rewriting rule 2"
      << std::endl
