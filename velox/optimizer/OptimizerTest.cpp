...
OptOutput optiming_plan(PlanBuilder& planBuilder, DataFrame data, int num_samples, int features_size, int first_layer_size, int second_layer_size){
  ...
  if (dyDecision.replaceFlag) {
    ...
    auto planBuilerOpt = build_plan_op(data.weights[0], dyDecision.rightBlockSize[0], dyDecision.rightBlockSize[1], inputs.schema, weights.schema, decisions.targetStr);
    output.flag = true;
    output.inputsPaths = inputs.paths;
    output.weightPaths = weights.paths;
    output.planBuilderOpt = planBuilerOpt;
    return output;
  }
  else {
    output.flag = false;
    output.planBuilderOpt = planBuilder;
    return output;
  }
}
...
void test_optimizer_demo(argc, argv){
