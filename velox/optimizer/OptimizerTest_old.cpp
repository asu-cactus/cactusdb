...
void data_loader(){
    ...
    VectorMaker maker{pool_.get()};
    int input_size = 597540; // num_features
    int layer1_size = 1024; // num units in hidden layer 1
    int layer2_size = 14588; // cat
...
FileStructure block_to_files(std::vector<std::vector<float>> valuesArray, int parts, int flag){
  ...
  if (flag == 0){
    ...
    for (int i = 0; i < parts; i++){
      auto input = maker.rowVector({"v", "v_row", "v_col"}, 
  ...
  return myFileStructure;
    ...
    auto indexs = create_block_index(parts, flag);
    for (int i = 0; i < parts; i++){
      auto input = maker.rowVector({"w", "w_row", "w_col"}, 
      {maker.arrayVector<float>({valuesArray[i]}, REAL()), maker.flatVector({indexs[0][i]}), maker.flatVector({indexs[1][i]})});
      auto file = TempFilePath::create();
      myFile.writeToFile(file->path, {input});
      paths.push_back(file);
...
PlanBuilder& build_plan_udf(DataFrame data, int features, int first_layer, int second_layer){
  VectorMaker maker{pool_.get()};
...
struct OptOutput {
  ...
  PlanBuilder& planBuilderOpt;

  OptOutput(bool fl, std::vector<std::shared_ptr<TempFilePath>> iPaths, std::vector<std::shared_ptr<TempFilePath>> wPaths, PlanBuilder& builder) : flag(fl), planBuilderOpt(builder), inputsPaths(iPaths), weightPaths(wPaths) {}
}
...
DynamicMetaData decision_maker(PlanBuilder& planBuilder){
  //Todo: add true dynamic tool
    ...
    decisions.planIdShot = {0, 1, 0, 0, 1};
    decisions.leftBlockSize.push_back(1000);
    decisions.leftBlockSize.push_back(149385);
    decisions.rightBlockSize.push_back(149385);
    decisions.rightBlockSize.push_back(1024);
    decisions.targetStr.push_back("mat_mul0(v)");
    return decisions;
}

PlanBuilder& build_plan_op(float* weight, int row, int col, RowTypePtr inputs, RowTypePtr weights, std::vector<std::string> str){
  exec::registerVectorFunction(
    "mat_mul_b",
    MatrixMultiply_b::signatures(),
    std::make_unique<MatrixMultiply_b>(row, col, weight)
  );
...
OptOutput optiming_plan(PlanBuilder& planBuilder, DataFrame data, int num_samples, int features_size, int first_layer_size, int second_layer_size){
  ...
  if (dyDecision.replaceFlag) {
    ...
    auto inputs = block_to_files(inputBlocks, blocksNum, 0);//0 denote values, 1 denote weight
    auto weights = block_to_files(weightBlocks, blocksNum, 1);
    auto planBuilderOpt = build_plan_op(data.weights[0], dyDecision.rightBlockSize[0], dyDecision.rightBlockSize[1], inputs.schema, weights.schema, dyDecision.targetStr);
    OptOutput optOutput(true, inputs.paths, weights.paths, planBuilderOpt);
    return optOutput;
  }
  else {
    OptOutput optOutput(false, {}, {}, planBuilder);
    return optOutput;
  }
}
...
std::vector<std::shared_ptr<TempFilePath>> weightPaths, int threadsNum){
  ...
  auto planOptAssert = planBuilderOpt.planNode();
  DuckDbQueryRunner duckDbQueryRunner_;
  auto results = exec::test::AssertQueryBuilder(planOptAssert, duckDbQueryRunner_)
  .splits(p2, myFile.makeHiveConnectorSplits(inputHiveSplits))
  .splits(p3, myFile.makeHiveConnectorSplits(weightHiveSplits))
  .copyResults(pool_.get());
  std::cout << "a1 values Results:" << results->toString() << std::endl;
  std::cout << results->toString(0, results->size()) << std::endl;
...
void test_optimizer_demo(int argc, char** argv){
  ...
  int memory_limit_udf = 38;//mb
  int splits_num_udf = 4;
  int threads_num_udf = 4;

  int memory_limit_rela = 102;//mb
  ...
  auto relational_plan = optiming_plan(udf_plan_builder, data, num_samples, input_features_size, first_layer_output_size, second_layer_output_size);
  exec_plan_relational(relational_plan.planBuilderOpt, memory_limit_rela, relational_plan.inputsPaths, relational_plan.weightPaths, threads_num_rela);
}
