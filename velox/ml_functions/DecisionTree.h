...
class Tree {
   ...
   public:
       ...
       static void constructTreeFromPath(std::string treePathIn, Node *tree) {
        ...
        constructTreeFromPathHelper(treePathIn, relationships, innerNodes, leafNodes);
	...
	float * outData = resultVector.data();
	 ...
	 std::cout << rowIndex << ":" << curIndex <<":" <<(float)(tree[curIndex].leafValue) << std::endl;
      ...
      inline void predictMissing(VectorPtr& input, std::vector<float>& resultVector, int numInputs, int numFeatures) {
        //get the input features
        auto inputFeatures = input->as<ArrayVector>()->elements();
        float* inputValues = inputFeatures->values()->asMutable<float>();
        float * outData = resultVector.data();
...
class TreePrediction : public MLFunction {
  ...
  void apply(
    ...
    if (hasMissing) {
      this->tree->predictMissing(args[0], resultVector, numInputs, this->numFeatures);     
    } else {
