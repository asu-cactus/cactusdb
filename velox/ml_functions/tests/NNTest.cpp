...
class MLFunctionsTest : public HiveConnectorTestBase {
 public:
  ...
  data_generate(int features, int samples, int first_layer, int second_layer) {
    ...
    // std::uniform_real_distribution<float> distribution(0.00000009,
    // 0.00000011); // Define the range
  ...
  std::shared_ptr<core::QueryCtx> newQueryCtx(int64_t memoryCapacity) {
    ...
    auto queryCtx = std::make_shared<core::QueryCtx>(
        ...
        /*
        Note from origin PR: Removing the relationship of AsyncDataCache
        inheritance from MemoryAllocator. Please check the following commit:
...
void MLFunctionsTest::torch_ffnn(
    int input_size,
    int layer1_size,
    int layer2_size) {
...
void MLFunctionsTest::traverse(std::shared_ptr<const core::PlanNode>& node) {
  ...
  std::cout << "Sources for " << node->name() << " - " << node->id()
            << std::endl;
