 ...
 * In the first test case, called testingTreePredictSmall(), we test the query of "SELECT decision_tree_predict(x) FROM data1";
 * In the second test case, called testingForestPredictSmall(), we test the query of "SELECT decision_forest_predict(x) FROM data2";
 */
...
class MLSQLTest : public HiveConnectorTestBase {
 public:
  ...
  //The DuckDbQueryPlanner instance, it will convert SQL query into a Velox plan.
...
void MLSQLTest::testingTreePredictSmall() {
  ...
  this->planner_.registerScalarFunction("decision_tree_predict", {ARRAY(REAL())}, REAL());
...
void MLSQLTest::testingForestPredictSmall() {
  ...
  this->planner_.registerScalarFunction("decision_forest_predict", {ARRAY(REAL())}, REAL());
