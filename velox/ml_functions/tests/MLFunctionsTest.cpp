...
class MLFunctionsTest : public HiveConnectorTestBase {
 public:
  ...
  // std::shared_ptr<memory::MemoryPool> childPool =
...
void MLFunctionsTest::test_mat_mul() {
  ...
  exec::registerVectorFunction(
      ...
      std::make_unique<MatrixMultiply>(
          weights->values()->asMutable<float>(), input_size, output_size));
  ...
  auto results =
      exec::test::AssertQueryBuilder(myPlan)
          .maxDrivers(4)
  ...
  std::cout << "Time for Matrix multiply (sec) = "
...
void MLFunctionsTest::test_relu() {
  std::cout << "==================" << std::endl;
  std::cout << "Test case of ReLu" << std::endl;
  int num_rows = 1000;
  int num_cols = 5000;
...
void MLFunctionsTest::test_softmax() {
  ...
  // step1: Register
  exec::registerVectorFunction(
      "softmax", Relu::signatures(), std::make_unique<Softmax>());
...
void MLFunctionsTest::test_argmax() {
  std::cout << "==================" << std::endl;
  std::cout << "Test case of Argmax" << std::endl;
  ...
  // step1: Register
  exec::registerVectorFunction(
      "argmax", Argmax::signatures(), std::make_unique<Argmax>());
...
void MLFunctionsTest::test_dense_layer() {
  std::cout << "==================" << std::endl;
  std::cout << "Test case of Dense Layer" << std::endl;
  ...
  exec::registerVectorFunction(
      ...
      MatrixAddition::signatures(),
      std::make_unique<MatrixAddition>(
          bias->values()->asMutable<float>(), output_size));
  ...
  std::string compute =  NNBuilder()
                          .denseLayer(5,10,weights->values()->asMutable<float>(),
  bias->values()->asMutable<float>(), NNBuilder::RELU) .build();
...
void MLFunctionsTest::test_torch_dense_layer() {
  ...
  // std::ifstream
  // weights_file("../../../../velox/ml_functions/tests/weights.txt");
  // std::ifstream bias_file("../../../../velox/ml_functions/tests/bias.txt");
  // std::ifstream
  // test_file("../../../../velox/ml_functions/tests/test_samples.txt");
  ...
  FlatVectorPtr<float> weights_1 =
      get_tensor(weights_file, layer1_size * input_size, input_size);
  FlatVectorPtr<float> bias_1 = get_tensor(bias_file, layer1_size, 1);
  FlatVectorPtr<float> weights_2 =
      get_tensor(weights_file, layer2_size * layer1_size, layer1_size);
  FlatVectorPtr<float> bias_2 = get_tensor(bias_file, layer2_size, 1);
  weights_file.close();
  bias_file.close();

  FlatVectorPtr<float> input =
      get_tensor(test_file, input_size * num_samples, num_samples);
  ...
  float* weights[2] = {
      weights_1->values()->asMutable<float>(),
      weights_2->values()->asMutable<float>()};
  float* bias[2] = {
      bias_1->values()->asMutable<float>(),
      bias_2->values()->asMutable<float>()};
  ...
  auto hiveSplits = makeHiveConnectorSplits(
      file->path, num_splits, dwio::common::FileFormat::DWRF);
  ...
  std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
  std::stringstream ss;
  ss << num_samples << "," << num_splits << "," << velox_threads << ","
     << torch_threads << ",";
  std::cout << ss.str()
            << (std::chrono::duration_cast<std::chrono::microseconds>(
                    end - begin)
...
void MLFunctionsTest::test_mnist() {
  ...
  FlatVectorPtr<float> bias_1_mat =
      maker.flatVector<float>(num_samples * layer1_size);
  for (int i = 0; i < bias_1_mat->size(); i++)
    bias_1_mat->set(i, bias_1_values[i % layer1_size]);

  FlatVectorPtr<float> bias_2_mat =
      maker.flatVector<float>(num_samples * layer2_size);
  ...
  std::string compute = NNBuilder()
                            .denseLayer(
                                ...
                                input_size,
                                weights_1->values()->asMutable<float>(),
                                bias_1_mat->values()->asMutable<float>(),
                                NNBuilder::RELU)
                            .denseLayer(
                                layer2_size,
                                layer1_size,
                                weights_2->values()->asMutable<float>(),
                                bias_2_mat->values()->asMutable<float>(),
                                NNBuilder::SOFTMAX)
                            .build();
  ...
  std::cout
      << compute
      << std::
             endl; // softmax5(mat_add4(mat_mul3(relu2(mat_add1(mat_mul0({}))))))
  auto plan = exec::test::PlanBuilder(pool_.get())
                  .values({inputRowVector})
                  .project({fmt::format(compute, "x")})
                  .planNode();
...
FlatVectorPtr<float>
MLFunctionsTest::get_tensor(std::ifstream& file, int size, int lines) {
  return get_tensor(maker, file, size, lines);
}

FlatVectorPtr<float> MLFunctionsTest::get_tensor(
    VectorMaker& m,
    std::ifstream& file,
    int size,
    int lines) {
...
void MLFunctionsTest::test_multithreading() {
  ...
  auto hiveSplits =
      makeHiveConnectorSplits(file->path, 3, dwio::common::FileFormat::DWRF);
  int concurrency = 3;
...
void MLFunctionsTest::mytest() {
  ...
  struct {
    ...
    std::string debugString() const {
      ...
      // Memory limit is disabled so spilling is not triggered.
      {0, false},
...
void MLFunctionsTest::test_multithreading_oom() {
  ...
  // queryCtx_->testingOverrideConfigUnsafe(
  //     {{core::QueryConfig::kPreferredOutputBatchRows, "400"},
  //     {core::QueryConfig::kPreferredOutputBatchBytes, "2000000"},
  //     {core::QueryConfig::kMaxOutputBatchRows, "300"}});
  // Create task
  std::shared_ptr<memory::MemoryPool> rootPool{
      memory::MemoryManager::getInstance()->addRootPool("root", 500 * MB)};
  queryCtx_->testingOverrideMemoryPool(rootPool);
...
void MLFunctionsTest::test_batching() {
  ...
  //{core::QueryConfig::kPreferredOutputBatchBytes, "2000000"}
  queryCtx_->testingOverrideConfigUnsafe(
      {{core::QueryConfig::kPreferredOutputBatchBytes, "4000000"},
       {core::QueryConfig::kMaxOutputBatchRows, "300"}});
  // Create task
  std::shared_ptr<memory::MemoryPool> rootPool{
      memory::MemoryManager::getInstance()->addRootPool("root", 500 * MB)};
...
void MLFunctionsTest::test_spill(
    int numDriver = 4,
    int memoryPoolSizeMB = 100,
    int spillMemThresholdMB = 20,
    bool enableSpill = false,
    int repeatRun = 5) {
  std::cout
      << fmt::format(
             "[INFO] Test Spill, memoryPoolSizeMB: {}, spillMemThresholdMB: {}, enable_spill: {}",
             memoryPoolSizeMB,
             spillMemThresholdMB,
             enableSpill)
      << std::endl;
  ...
  auto leftRowType = ROW({"l1", "l2"}, {INTEGER(), INTEGER()});

  auto rightRowType = ROW({"r1", "r2"}, {INTEGER(), INTEGER()});

  auto tempPath = exec::test::TempDirectoryPath::create();
  auto leftFilePath = fs::path(fmt::format("{}/left.parquet", tempPath->path));
  ...
  if (enableSpill) {
    qctx->testingOverrideConfigUnsafe({
        {core::QueryConfig::kSpillEnabled, "true"},
        {core::QueryConfig::kJoinSpillEnabled, "true"},
        {core::QueryConfig::kJoinSpillMemoryThreshold,
         std::to_string(spillMemThresholdMB * MB)},
    });
  }
  ...
  auto rightDataHiveSplits = makeHiveConnectorSplits(
      {rightFilePath}, 5, dwio::common::FileFormat::PARQUET);

  auto leftDataHiveSplits = makeHiveConnectorSplits(
      {leftFilePath}, 5, dwio::common::FileFormat::PARQUET);
  ...
  for (int i = 0; i < repeatRun; i++) {
    ...
    auto addSplits = [&noMoreSplits,
                      &rightDataHiveSplits,
                      &rightTableScanNodeId,
                      &leftDataHiveSplits,
                      &leftTableScanNodeId](exec::Task* task) {
      if (!noMoreSplits) {
...
void MLFunctionsTest::test_torch_dense_layer_multithreading() {
  std::cout << "==================" << std::endl;
  std::cout << "Test case of Torch Dense Layer" << std::endl;
  ...
  config->set(
      ...
      (uint32_t)(num_samples / num_splits));
  auto file = TempFilePath::create();
  writeToFile(file->path, {inputRowVector}, config);
  ...
  queryCtx_->testingOverrideConfigUnsafe(
      {{core::QueryConfig::kPreferredOutputBatchBytes,
        std::to_string((int)confs[3])},
       {core::QueryConfig::kMaxOutputBatchRows,
        std::to_string((int)confs[4])}});
  auto task = exec::Task::create(
      "0",
      plan,
      0,
...
void MLFunctionsTest::test_mnist_oom_weights() {
  ...
  std::string compute =
      NNBuilder(weights_file_name, bias_file_name)
  ...
  // total memory is 5MB
  std::shared_ptr<memory::MemoryPool> rootPool{
      memory::MemoryManager::getInstance()->addRootPool("root", 4 * MB)};
  ...
  auto mat_mul0 = std::dynamic_pointer_cast<MatrixMultiply>(
      exec::getVectorFunction("mat_mul0", {ARRAY(REAL())}, {}, config));
  auto mat_add1 = std::dynamic_pointer_cast<MatrixAddition>(
      exec::getVectorFunction("mat_add1", {ARRAY(REAL())}, {}, config));
  auto mat_mul3 = std::dynamic_pointer_cast<MatrixMultiply>(
      exec::getVectorFunction("mat_mul3", {ARRAY(REAL())}, {}, config));
  auto mat_add4 = std::dynamic_pointer_cast<MatrixAddition>(
      exec::getVectorFunction("mat_add4", {ARRAY(REAL())}, {}, config));
  ...
  FlatVectorPtr<float> weights_1 =
      get_tensor(m, weights_file, layer1_size * input_size, input_size);
  FlatVectorPtr<float> bias_1 = get_tensor(m, bias_file, layer1_size, 1);
  FlatVectorPtr<float> weights_2 =
      get_tensor(m, weights_file, layer2_size * layer1_size, layer1_size);
  ...
  FlatVectorPtr<float> bias_1_mat =
      m.flatVector<float>(num_samples * layer1_size);
  for (int i = 0; i < bias_1_mat->size(); i++)
    bias_1_mat->set(i, bias_1_values[i % layer1_size]);

  FlatVectorPtr<float> bias_2_mat =
      m.flatVector<float>(num_samples * layer2_size);
  for (int i = 0; i < bias_2_mat->size(); i++)
    bias_2_mat->set(i, bias_2_values[i % layer2_size]);
...
void MLFunctionsTest::test_conv2d() {
  ...
  int weights1_size = cnn_layer1_filter_dims[0] * cnn_layer1_filter_dims[1] *
      cnn_layer1_filter_dims[2] * cnn_layer1_filters;

  int cnn_layer2_filters = 64;
  int cnn_layer2_filter_dims[] = {3, 3, 64};
  int weights2_size = cnn_layer2_filter_dims[0] * cnn_layer2_filter_dims[1] *
      cnn_layer2_filter_dims[2] * cnn_layer2_filters;
  ...
  int dims1[] = {
      cnn_layer1_filters,
      cnn_layer1_filter_dims[0],
      cnn_layer1_filter_dims[1],
      cnn_layer1_filter_dims[2],
      input_dims[0],
      input_dims[1]};
  int dims2[] = {
      cnn_layer2_filters,
      cnn_layer2_filter_dims[0],
      cnn_layer2_filter_dims[1],
      cnn_layer2_filter_dims[2],
      input_dims[0] - cnn_layer1_filter_dims[0] + 1,
      input_dims[1] - cnn_layer1_filter_dims[1] + 1};
  ...
  FlatVectorPtr<float> weights_1 = get_tensor(
      weights_file,
      weights1_size,
      cnn_layer1_filters * cnn_layer1_filter_dims[2]);
  FlatVectorPtr<float> bias_1 = get_tensor(bias_file, cnn_layer1_filters, 1);

  FlatVectorPtr<float> weights_2 = get_tensor(
      weights_file,
      weights2_size,
      cnn_layer2_filters * cnn_layer2_filter_dims[2]);
  ...
  FlatVectorPtr<float> weights_3 =
      get_tensor(weights_file, layer3_size * input3_size, input3_size);
  FlatVectorPtr<float> bias_3 = get_tensor(bias_file, layer3_size, 1);
  FlatVectorPtr<float> weights_4 =
      get_tensor(weights_file, layer4_size * layer3_size, layer3_size);
  ...
  FlatVectorPtr<float> bias_3_mat =
      maker.flatVector<float>(num_samples * layer3_size);
  for (int i = 0; i < bias_3_mat->size(); i++)
    bias_3_mat->set(i, bias_3_values[i % layer3_size]);

  FlatVectorPtr<float> bias_4_mat =
      maker.flatVector<float>(num_samples * layer4_size);
  for (int i = 0; i < bias_4_mat->size(); i++)
    bias_4_mat->set(i, bias_4_values[i % layer4_size]);

  FlatVectorPtr<float> input = get_tensor(
      test_file, input_size * num_samples, num_samples * input_dims[2]);
  float* data = input->values()->asMutable<float>();
  ...
  std::string compute = NNBuilder()
                            ...
                            .maxPoolLayer(
                                2,
                                dims2[4] - cnn_layer2_filter_dims[0] + 1,
                                dims2[5] - cnn_layer2_filter_dims[1] + 1)
                            .denseLayer(
                                layer3_size,
                                input3_size,
                                weights_3->values()->asMutable<float>(),
                                bias_3_mat->values()->asMutable<float>(),
                                NNBuilder::RELU)
                            .denseLayer(
                                layer4_size,
                                layer3_size,
                                weights_4->values()->asMutable<float>(),
                                bias_4_mat->values()->asMutable<float>(),
                                NNBuilder::SOFTMAX)
                            .build();
...
void MLFunctionsTest::test_deep_bench_conv1() {
  ...
  int weights_size = cnn_filter_dims[0] * cnn_filter_dims[1] *
      cnn_filter_dims[2] * cnn_filters;
  ...
  FlatVectorPtr<float> weights =
      get_tensor(weights_file, weights_size, cnn_filters * cnn_filter_dims[2]);
  ...
  exec::registerVectorFunction(
      "torchConvolute",
      TorchDNN2Level::signatures(),
      std::make_unique<TorchConvolute>(
          weights->values()->asMutable<float>(), dims));
...
void MLFunctionsTest::test_complex_torchnn() {
  ...
  auto results = exec::test::AssertQueryBuilder(myPlan.planNode())
                     .copyResults(pool_.get());
...
void MLFunctionsTest::run(
  ...
  //  test_spill(numDriver, memoryPoolSizeMB, spillMemThresholdMB, enableSpill,
  //  repeatRun); mytest(); test_mnist_multithreading();
