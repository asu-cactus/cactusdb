...
void FraudDetectionTest::registerFunctions(
    std::string modelFilePath,
    int numCols) {
  ...
  std::cout << "Completed registering function for date_to_timestamp_1"
            << std::endl;
  ...
  std::cout << "Completed registering function for date_to_timestamp_2"
            << std::endl;
  ...
  std::cout << "Completed registering function for time_diff_in_days"
            << std::endl;
  ...
  std::cout << "Completed registering function for get_transaction_features"
            << std::endl;
  ...
  std::cout << "Completed registering function for get_customer_features"
            << std::endl;
  ...
  exec::registerVectorFunction(
      "concat_vectors2", Concat::signatures(), std::make_unique<Concat>(4, 5));
  ...
  std::cout << "Completed registering function for xgboost_fraud_predict"
            << std::endl;

  std::string xgboost_fraud_transaction_path =
      "resources/model/fraud_xgboost_5_16";
  exec::registerVectorFunction(
      "xgboost_fraud_transaction",
      ForestPrediction::signatures(),
      std::make_unique<ForestPrediction>(
          xgboost_fraud_transaction_path, 5, true));
  std::cout << "Completed registering function for xgboost_fraud_transaction"
            << std::endl;
}

void FraudDetectionTest::registerNNFunctions(int numCols) {
  std::vector<std::vector<float>> w1 =
      loadHDF5Array("resources/model/fraud_dnn_weights.h5", "fc1.weight", 0);
  std::vector<std::vector<float>> b1 =
      loadHDF5Array("resources/model/fraud_dnn_weights.h5", "fc1.bias", 0);
  std::vector<std::vector<float>> w2 =
      loadHDF5Array("resources/model/fraud_dnn_weights.h5", "fc2.weight", 0);
  std::vector<std::vector<float>> b2 =
      loadHDF5Array("resources/model/fraud_dnn_weights.h5", "fc2.bias", 0);
  std::vector<std::vector<float>> w3 =
      loadHDF5Array("resources/model/fraud_dnn_weights.h5", "fc3.weight", 0);
  std::vector<std::vector<float>> b3 =
      loadHDF5Array("resources/model/fraud_dnn_weights.h5", "fc3.bias", 0);
  ...
  exec::registerVectorFunction(
      "relu", Relu::signatures(), std::make_unique<Relu>(), {}, true);

  exec::registerVectorFunction(
      "softmax", Softmax::signatures(), std::make_unique<Softmax>());
}

ArrayVectorPtr FraudDetectionTest::parseCSVFile(
    VectorMaker& maker,
    std::string filePath,
    int numRows,
    int numCols) {
...
RowVectorPtr FraudDetectionTest::writeDataToFile(
    std::string csvFilePath,
    int numRows,
    int numCols,
    int numDataSplits,
  ...
  ArrayVectorPtr inputArrayVector =
      parseCSVFile(maker, csvFilePath, numRows, numCols);
  ...
  auto inputRowVector =
      maker.rowVector({"row_id", "x"}, {inputIndexVector, inputArrayVector});
  ...
  dataConfig->set(
      facebook::velox::dwrf::Config::STRIPE_SIZE, 799 * kDataSizeKB);
...
std::vector<std::vector<float>> FraudDetectionTest::loadHDF5Array(
    const std::string& filename,
    const std::string& datasetName,
    int doPrint) {
...
void FraudDetectionTest::testingWithRealData(
    int numDataSplits,
    int dataBatchSize,
    int numRows,
    int numCols,
  ...
  RowVectorPtr transactionRowVector =
      getTransactionData("resources/data/financial_transactions.csv");
  std::cout << "transactionRowVector data generated" << std::endl;
  RowVectorPtr customerRowVector =
      getCustomerData("resources/data/customer.csv");
  std::cout << "customerRowVector data generated" << std::endl;

  registerNNFunctions(9);

  auto dataHiveSplits = makeHiveConnectorSplits(
      path, numDataSplits, dwio::common::FileFormat::DWRF);
  ...
  auto myPlan =
      exec::test::PlanBuilder(planNodeIdGenerator, pool_.get())
          ...
          .project(
              {"transaction_id",
               "concat_vectors2(customer_features, transaction_features) AS all_features"})
          .project(
              {"transaction_id",
               "all_features",
               "softmax(mat_vector_add_3(mat_mul_3(relu(mat_vector_add_2(mat_mul_2(relu(mat_vector_add_1(mat_mul_1(all_features))))))))) AS fraudulent_probs"})
...
DEFINE_string(
    dataFilePath,
    "resources/data/creditcard_test.csv",
    "path to input dataset to be predicted");
DEFINE_string(
    modelFilePath,
    "resources/model/fraud_xgboost_1600_8",
    "path to the model used for prediction");
DEFINE_string(
    orderDataFilePath,
    "resources/data/order.csv",
    "path to input order dataset");
...
int main(int argc, char** argv) {
  ...
  std::cout
      << fmt::format(
             "Option: {}, numDataSplits: {}, numTreeSplits: {}, numTreeRows: {}, dataBatchSize: {}, numRows: {}, numCols: {}, dataFilePath: {}, modelFilePath: {}, orderDataFilePath: {}",
             option,
             numDataSplits,
             numTreeSplits,
             numTreeRows,
