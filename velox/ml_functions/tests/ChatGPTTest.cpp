...
void ChatGPTTest::LLMWithoutOptimization() {
  ...
  //          REAL(),    REAL(), REAL(), REAL(), REAL(), VARCHAR()});
  auto userDataRowType =
      ROW({"user_id", "description"},
          {INTEGER(), VARCHAR()});
  ...
  exec::registerVectorFunction(
      "chatgpt_recommender", ChatGPTRecommender::signatures(), std::make_unique<ChatGPTRecommender>());
  ...
  auto addSplits = [&](exec::Task* task) {
    ...
    if (!noMoreSplits) {
      auto userHiveSplits = makeHiveConnectorSplits(userDataPaths, dwio::common::FileFormat::PARQUET);
      ...
      auto movieHiveSplits = makeHiveConnectorSplits(movieDataPaths, dwio::common::FileFormat::PARQUET);
