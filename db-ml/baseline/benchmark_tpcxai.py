...
def benchmark_tpcxai_usecase03_tf(num_loop=10, **kwargs):
    llm_recommendation_pipeline = pipeline.TPCxAIUsecase03PipelineTF(
        num_loop=num_loop
    )
...
def benchmark_tpcxai_usecase10_tf(num_loop=10, **kwargs):
    llm_recommendation_pipeline = pipeline.TPCxAIUsecase10PipelineTF(
        num_loop=num_loop
    )
    benchmark_result = llm_recommendation_pipeline.run_pipeline()
    return benchmark_result
...
def benchmark_tpcxai():
    ...
    list_run_config = list(itertools.product(list_num_user, list_num_movie, list_benchmark))
